...
__all__ = (
        'counts_data_from_plots_outputs_data',
        )
...
# Main
def counts_data_from_plots_outputs_data(outputs_data: OutputsData,
        rows: int) -> CountsData:
...
def _counts_data_from_by_elm(elm: ElmType, titles: list, descs: list,
    ...
    for title, text in zip(titles[1:], descs[1:]):
        tmp.append(_record_count_from(elm, title, _rid_plot_mark(text), rows))
